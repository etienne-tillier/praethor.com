...
/**
 * Extracts available language links from a blog post.
 * This is a headless utility — the agent should wrap the output in their own styled component.
 */
export function getAvailableLanguages(post: BlogPost, currentSlug: string): LanguageLink[] {
    ...
    if (post.default_locale) {
        ...
        links.push({ lang: "fr", slug: post.slug, label: "Français" });
    }
...
/**
 * Default LanguageSwitcher component with minimal styling.
 * The agent can replace this with their own styled version.
 */
        ...
        <div className={`flex gap-3 items-center text-sm ${className}`}>
            ...
            {otherLinks.map((l) => (
                <Link
                    key={l.lang}
                    href={`/blog/${l.slug}`}
                    className="underline decoration-1 underline-offset-2 hover:opacity-80 transition-opacity uppercase font-medium"
                >
                    {l.label}
                </Link>
            ))}
